...
contract SMT {
  ...
  function write(uint160 key, bytes32 prevLeaf, bytes memory proof, bytes32 newLeaf) public returns(bytes32)  {
  ...
  function del(uint160 key, bytes32 prevLeaf, bytes memory proof) public returns(bytes32) {
    bytes32 calculatedRoot = getRoot(prevLeaf, key, proof);
    require(calculatedRoot == root, "update proof not valid");
  ...
  function getRoot(bytes32 leaf, uint160 _index, bytes memory proof) public pure returns (bytes32) {
